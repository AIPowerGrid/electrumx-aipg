...
Not started until the Block Processor has caught up with aipgd.

Daemon
------

Encapsulates the RPC wire protocol with aipgd for the whole server.
Transparently handles temporary aipgd connection errors, and fails
over if necessary.
...
from aipgd.  Once it has caught up it additionally asks the Daemon
to refresh its view of aipgd's mempool transaction hashes.  Serves
blocks to the Block Processor via a queue.

Mempool
-------

Handles all the details of maintaining a representation of aipgd's
